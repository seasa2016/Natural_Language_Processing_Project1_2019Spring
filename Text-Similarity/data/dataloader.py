...
class itemDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if(transforms):
            ...
            sample['label'][-1] = torch.tensor(sample['label'][-1],dtype=torch.long)
            if(self.pred=='linear_two_class'):
                sample['label'][0] = torch.tensor(sample['label'][0],dtype=torch.float)
                sample['label'][1] = torch.tensor(sample['label'][1],dtype=torch.float)
            elif(self.pred=='linear_two_regression'):
                sample['label'][0] = torch.tensor(sample['label'][0],dtype=torch.long)
                sample['label'][1] = torch.tensor(sample['label'][1],dtype=torch.long)
