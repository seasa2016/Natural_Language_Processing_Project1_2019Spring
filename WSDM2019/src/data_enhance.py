...
if __name__ == '__main__':
    ...
    train_data = pd.read_csv(basedir + 'train.csv', sep=',')
    print(train_data.shape)
    train_data.fillna(' ', inplace=True)
    for index in train_data.index:
        label = train_data.loc[index].values[-1]
        ...
        nodes.add(zh2)
        #print(zh1,zh2)
        agreed_set.add('{0}--{1}'.format(zh1,zh2))
        agreed_set.add('{0}--{1}'.format(zh2,zh1))
    ...
    for path in paths:
        ...
        for pair in agreedPairsGenerator:
            if len(pair) != 2:
                ...
                continue
            a = '{0}--{1}'.format(pair[0],pair[1])
            b = '{0}--{1}'.format(pair[1],pair[0])
    ...
    for index in train_data.index:
        ...
        if label != 'disagreed':
            continue
        dis_count += 1
        exist.add("{0}--{1}".format(zh1,zh2))
        exist.add("{0}--{1}".format(zh2,zh1))
    ...
    for index in train_data.index:
        ...
        for path in paths:
            if zh1 in path:
                for item in path:
                    a = "{0}--{1}".format(item,zh2)
                    b = "{0}--{1}".format(zh2,item)
                    if a not in exist:
            ...
            if zh2 in path:
                for item in path:
                    a = "{0}--{1}".format(item,zh1)
                    b = "{0}--{1}".format(zh1,item)
                    if a not in exist:
                        new_dis.add(a)
                    # else:
                    #     print(a)
